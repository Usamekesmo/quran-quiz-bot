...
def begin_quiz(update: Update, context: CallbackContext):
    aya_sample = random.sample(questions_aya, 10)
    complete_sample = random.sample(questions_complete, 10)
    combined = [{'type': 'aya', 'data': q} for q in aya_sample] + [{'type': 'complete', 'data': q} for q in complete_sample]
...
def handle_answer(update: Update, context: CallbackContext):
    if context.user_data.get('awaiting_name'):
        ...
        keyboard = [['📝 ابدأ الاختبار']]
        reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
        update.message.reply_text(f"مرحبًا {name}! اضغط على الزر أدناه لبدء الاختبار. 🧠", reply_markup=reply_markup)
        return
...
def main():
    ...
    dp.add_handler(MessageHandler(Filters.regex('^📝 ابدأ الاختبار$'), begin_quiz))
